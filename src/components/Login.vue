<template>
  <div class="login_container">
    <div class="login_box">
        ...
        <!--登录表单区域-->
        <el-form ref="loginFormRef" :model="loginForm" :rules="loginFormRules" label-width="0px" class="login_form">  <!--这是使用element-ui的第三步，完成。:model="loginForm"表示该表单填写的数据都会自动同步到loginForm对象上。:rules表单验证。ref表示该表单的实例对象-->
          <el-form-item label="" prop="username">
            <el-input v-model="loginForm.username"  prefix-icon="iconfont icon-bussiness-man"></el-input><!--这是使用字体图标的第二步，完成，使用阿里图标图必须写iconfont-->
          </el-form-item>
...
export default {
  ...
  methods: {
    ...
    login(){
      this.$refs.loginFormRef.validate(valid => {   //对整个表单进行校验的方法，参数为一个回调函数。该回调函数会在校验结束后被调用，并传入两个参数：是否校验成功和未通过校验的字段。valid是一个boolean值，是该验证的结果,
        if(!valid) return;
        let _this = this;
...
<style lang="less" scoped> //less表示一种语法；scoped表示这个style在只在当前组件生效
.login_container{
